...
const readMessages = Firebase.functions.https.onCall(
	async ({ messageIds, threadId }: ReadMessageData, context) => {
		...
		const seenMessages = tobeSeenMessages.map((msg) => {
			...
			updatedSeenAt[readerUid] = Firebase.serverTime() as Timestamp;
			return {
				...msg,
				time: parsedTime,
		...
		if (seenMessages.length > 0) {
			...
			// by function invoker
			batch.update(Firebase.db.collection('threads').doc(threadId), {
