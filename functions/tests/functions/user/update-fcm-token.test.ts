...
export const updateFCMToken = async (ctx: Context, opts: any) => {
	await ctx.firebase.signOut();

	try {
		await ctx.firebase.functions('updateFCMToken')(opts);
	} catch (error) {
	...
	try {
		await ctx.firebase.functions('updateFCMToken')({
			a: 'invalid data',
		});
	} catch (error) {
		assert(error instanceof FirebaseError);
	...
	const res = await ctx.firebase.functions('updateFCMToken')(opts);
